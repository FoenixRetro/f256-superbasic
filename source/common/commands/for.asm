...
ForCommand: ;; [for]
                ...
                lda     (codePtr),y
                sta     zTemp2                  ; save keyword for later
                iny                             ; consume it
                cmp     #KWD_DOWNTO
                beq     _FCNoSyntax
                cmp     #KWD_TO
...
_FCNoSyntax:
                ...
                jsr     EvaluateInteger
                ;
                ; Now set the +1 or -1 increment for the TO or DOWNTO
                ;
                phy                             ; save current position
                lda     zTemp2                  ; restore DOWNTO or TO
...
_FCNotDownTo:                                   ; 0 if DOWNTO 2 if TO
                ...
                sta     (basicStack),y          ; store -1 or 1 in the Basic Stack.
                bmi     _FCNegativeStep
...
_FCNegativeStep:
                ...
                jsr     FCIntegerToStack
                ;
                ; Handle optional STEP value for use with TO only
                ;
                ply                             ; restore position
                lda     (codePtr),y
                cmp     #KWD_STEP               ; check for optional STEP
                bne     _FCNoStep
                iny                             ; consume STEP
                ;
                ldx     #0
                ...
                cmp     #KWD_DOWNTO
                bne    _FCStoreStep
                jsr     NSMNegateMantissa       ; if DOWNTO, negate the step value

_FCStoreStep:
                phy                             ; save the new position
                ldy     #16                     ; set the step value
                ldx     #0                      ; FIXME: not needed if x was unchanged
                jsr     FCIntegerToStack
                ply                             ; restore position
_FCNoStep:
                ;
...
NextCommand: ;; [next]
                ...
                ;
                lda     basicStack              ; Pointer to step
                clc
                adc     #8
                sta     zTemp1
                lda     basicStack+1
...
;               Date                    Notes
;               ====                    =====
;               17/06/25                Added support for optional use of STEP
;
; ************************************************************************************************
